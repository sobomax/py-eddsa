...
def native_arch_args(prefix=""):
    machine = platform.machine().lower()
    if machine in {"x86_64", "amd64", "i386", "i686", "x86"}:
        return [f"{prefix}-march=native"]
    if machine in {"aarch64", "arm64"}:
        return [f"{prefix}-mcpu=native"]
    return []
...
def compile_args():
    if get_platform().startswith("win"):
        if os.environ.get("CC", "").lower() == "clang-cl":
            return (
                ["/std:clatest", "/O2", "/GL", *native_arch_args("/clang:")],
                ["/LTCG", "-fuse-ld=lld"],
            )
    ...
    c_args.extend(native_arch_args())
    return c_args, ["-flto"]
